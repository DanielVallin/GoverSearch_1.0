...
class cReg_Leyes extends CI_Controller {
	...
	public function index()
	{
		...
		$this->load->view("admin/Menu/vReg_Leyes/list", $data);
	...
	public function store(){
		...
		if ($this->form_validation->run()==TRUE) {
			...
			if ($this->Reg_Leyes_Model->save($data)) {
				redirect(base_url()."Menu/cReg_Leyes");
			}else{
				$this->session->set_flashdata("error","No se pudo guardar la informacion");
				redirect(base_url()."Menu/cReg_Leyes/add");
			}
		}else{
			$this->add();
		}
	...
	public function update(){
		...
		if ($this->form_validation->run()==TRUE) {
			...
			if($this->Reg_Leyes_Model->update($pk_Datos_Ley, $data)){
				...
				redirect(base_url()."Menu/cReg_Leyes/edit/".$pk_Gpo_Ley);
			}
		}else{
